...
class LicenseChecker:
    def __init__(self) -> None:
        ...
        adapter = requests.adapters.HTTPAdapter(
            max_retries=urllib3.util.Retry(total=3, backoff_factor=0.5, status_forcelist=[500, 502, 503, 504])
        )
    ...
    def check_licenses(self, toml_file: Path) -> Dict[str, Dict[str, bool]]:
        ...
        for package in dependencies:
            ...
            if package in EXCEPTIONS:
                results[package] = {"license": "Approved Exception", "allowed": True}
                continue
            ...
            if normalized_license:
                allowed = (
                    normalized_license in {self.normalize_license(license) for license in ALLOWED_LICENSES}
                    or package in EXCEPTIONS
                )

            results[package] = {"license": license_info, "allowed": allowed}
