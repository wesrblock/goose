...
impl Agent {
    ...
    fn modify_messages(&self, messages: &mut Vec<Message>, status: String) -> AgentResult<()> {
    ...
    pub fn reply(&self, messages: &[Message]) -> BoxStream<'_, Result<Message>> {
        ...
        Box::pin(async_stream::try_stream! {
            loop {
                // Add the status message to the conversation history
                let status = self.get_system_status().await.map_err(|e| AgentError::Internal(e.to_string()))?;
                _ = self.modify_messages(&mut messages, status);
